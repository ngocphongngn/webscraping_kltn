...
const getTaxInfo = function(totalMoney, taxInfo, other) {
  ...
  if (taxInfo[0].TaxType == 1)
    for (var i in taxInfo) {
      if (
        other.contractOffical != 0 &&
        other.individualResident != 0 &&
        taxInfo[i].TaxStatus == 1
      ) {
        if (totalMoney >= taxInfo[i].StartAmount) {
          ...
          if (
            totalMoney <= taxInfo[i].FinishAmount ||
            taxInfo[i].FinishAmount === 0 ||
            taxInfo[i].FinishAmount == null
          ) {
            // Nếu gười nộp thuế là cá nhân cứ trú có hợp động trên 3 tháng
            result.money =
              (totalMoney * taxInfo[i].TaxPercent) / 100 -
              taxInfo[i].PartialAmount;
            if (taxInfo[i].TaxStatus == 1)
              result.taxPercent = `Lũy Tiến ${taxInfo[
                i
              ].TaxPercent.toString()} %`;
            return result;
          }
...
export { getTaxInfo, getSelfReduce };
